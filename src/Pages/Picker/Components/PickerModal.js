...
export default function PickerModal({
    ...
    const addHoliday = () => {
        if (validateCalendar()) {
            ...
            axios
                .post(`http://${process.env.REACT_APP_DB_ADDRESS}:3001/vacationers/${chosenVacationer.id}`, newHoliday)
                .then((response) => {
    ...
    const handleEdit = () => {
        ...
        axios
            .put(`http://${process.env.REACT_APP_DB_ADDRESS}:3001/vacationers/${chosenVacationer.id}/${idToEdit}`, editedHoliday)
            .then((response) => {
                console.log(response)
                resetForm();
                resetDates()
