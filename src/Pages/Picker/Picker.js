...
export default function Picker() {
    ...
    useEffect(() => {
        console.log("Saved!")
        axios
            .get(`http://${process.env.REACT_APP_ADDRESS}:3001/vacationers`)
            .then((response) => {
    ...
    const deleteHoliday = () => {
        axios.delete(`http://${process.env.REACT_APP_ADDRESS}:3001/vacationers/${chosenVacationer.id}/${holidayToDelete.id}`)
            .then(() => {
                setSave(!save);
                resetForm();
                resetDates()
    ...
    const calculatePerDay = (date1, date2) => {
        axios.get(`http://${process.env.REACT_APP_ADDRESS}:3001/timespan?start=${date1.toISOString()}&end=${date2.toISOString()}`)
            .then((response) => {
