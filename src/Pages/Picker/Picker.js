...
export default function Picker() {
    ...
    const deleteHoliday = () => {
        axios.delete(`${process.env.REACT_APP_ADDRESS}/vacationers/${chosenVacationer.id}/${holidayToDelete.id}`)
            .then(() => {
                setSave(!save);
                resetForm();
                resetDates()
    ...
    const calculatePerDay = (date1, date2) => {
        axios.get(`${process.env.REACT_APP_ADDRESS}/timespan?start=${date1.toISOString()}&end=${date2.toISOString()}`)
            .then((response) => {
