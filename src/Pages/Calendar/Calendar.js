...
export default function Calendar(props) {
    ...
    const getHolidaysOfMonth = (nextMonth) => {
        axios
            .get(
                `http://${process.env.REACT_APP_DB_ADDRESS}:3001/holidaysbetween?start=${selectedDate.toISOString()}&end=${nextMonth.toISOString()}`
            )
            .then((response) => {
    ...
    useEffect(() => {
        ...
        axios
            .get(`http://${process.env.REACT_APP_DB_ADDRESS}:3001/vacationers/total`)
            .then((response) => {
